#!/usr/bin/env python3
"""
Saltgrain miner. Standard library only -- no pip install, no dependencies.
...
def main():
    ...
    print(f"Saltgrain miner  ·  building on {state.tip_hash[:16]}…  ·  height {height}")
    print(f"  difficulty {difficulty(bits):,.1f}   bits {bits:#010x}")
    print(f"  puzzles    {k} x subset-sum(n={powfn.N})")
    print(f"  reward     {format_amount(reward)} SALT "

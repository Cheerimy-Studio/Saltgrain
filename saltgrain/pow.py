"""
Saltgrain (盐粒) proof of work: subset-sum.

Bitcoin's work function is "find a nonce whose header hashes below a target".
Saltgrain's is "find subsets of these numbers that sum exactly to these targets".

Both are cheap to verify and expensive to find. The difference is that
nobody has built an ASIC for subset-sum, so mining Saltgrain means writing a
...
def _half_sums(numbers):
    ...
    The result is a packed array of 64-bit integers rather than a list of
    Python ints. The sums never exceed 44 * 2^42 for any n this file allows,
    so they always fit, and the packed form uses roughly an eighth of the
    memory -- which matters because meet-in-the-middle is limited by memory,
    not time.
    """
    sums = array("q", [0])
    for a in numbers:
        sums.extend([s + a for s in sums])
    return sums

...
class UTXOSet:
    ...
    def __init__(self):
        ...
        # txid -> number of its outputs still unspent. Kept in step with
        # `utxos` so the BIP 30 check is a dict lookup rather than a scan of
        # every entry, which is what made replay crawl once the chain had a
        # few thousand unspent outputs.
        self.unspent_counts = {}

    def copy(self):
        u = UTXOSet()
        u.utxos = dict(self.utxos)
        u.unspent_counts = dict(self.unspent_counts)
        return u
    ...
    def add_tx(self, tx: Tx, height: int) -> None:
        ...
        self.unspent_counts[txid] = self.unspent_counts.get(txid, 0) + len(tx.outputs)

    def spend(self, txid: str, vout: int) -> None:
        del self.utxos[(txid, vout)]
        left = self.unspent_counts.get(txid, 0) - 1
        if left > 0:
            self.unspent_counts[txid] = left
        else:
            self.unspent_counts.pop(txid, None)
    ...
    def has_tx(self, txid: str) -> bool:
        ...
        return self.unspent_counts.get(txid, 0) > 0
...
def validate_tx(tx: Tx, utxos: UTXOSet, height: int) -> int:
    ...
    for i in tx.inputs:
        ...
        if not crypto.same_address(crypto.pubkey_to_address(pub_bytes), entry["address"]):
            raise ConsensusError(
...
def validate_block(
    ...
    if paid != expected_reward:
        raise ConsensusError(
            f"coinbase pays {paid} motes, expected exactly {expected_reward} "
...
def format_amount(motes: int) -> str:
    whole = motes // COIN
    frac = motes % COIN

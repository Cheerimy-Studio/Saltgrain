...
def main() -> int:
    ...
    for pr in prs:
        ...
        if not kind:
            close(n, "**Rejected.** No `salt-block-v1:`, `salt-tx-v1:` or "
                     "`salt-id-v1:` line in that file.", args.dry_run, pace)
            rejected += 1
            continue
        ...
        close(n, f"### Transaction queued\n\n`{tx.txid()}`\n\n"
                 f"Fee `{format_amount(fee)} SALT`. It waits for the next block.",
              args.dry_run, pace)
        queued += 1

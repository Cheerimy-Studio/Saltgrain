#!/usr/bin/env python3
"""
Mine the Saltgrain genesis block. Run once, before anything is pushed.

    python3 make_genesis.py --miner YOUR_HANDLE --message "..." --address salt1...
...
from saltgrain import crypto, render
...
def main() -> int:
    ...
    print(f"mining Saltgrain genesis at difficulty {difficulty(GENESIS_BITS):,.1f}")
    print(f"  puzzles  {k} x subset-sum(n={powfn.N})")
    print(f'  message  "{args.message}"')
    print(f"  reward   {format_amount(block_subsidy(0))} SALT -> {args.address}")
    ...
    print(f"  found in {elapsed:.1f}s ({k / elapsed:.1f} puzzles/s)")
    print()

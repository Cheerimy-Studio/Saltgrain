<h1 align="center">盐粒</h1>

<p align="center"><b>Saltgrain · SALT</b></p>

<p align="center">
  <a href="https://cheerimy-studio.github.io/Saltgrain/"><b>盐仓</b></a> ·
  <a href="../../issues/53"><b>采盐提交处</b></a> ·
  <a href="../../issues/2"><b>寄送提交处</b></a>
</p>

<p align="center"><sub>一条公开记录「盐粒」的账本。不是货币，不标价，不流通，不能兑换任何东西。</sub></p>

---

<!-- SALT:BEGIN -->

<picture><source media="(prefers-color-scheme: dark)" srcset="assets/ledger-dark.svg?v=2237"><img src="assets/ledger-light.svg?v=2237" width="100%" alt="Saltgrain ledger, height 2237"></picture>

| 项目 | 数值 |
|---|---|
| **高度** | `2237` |
| **链尖** | `58e1864dffbf71cb133c4a1cd08b2ad1e9f9f0deaeb65f95d1caa96a464a0170` |
| **难度** | `1,383.8`  (bits `0x1d02f5c6`) |
| **累计工作量** | `13,167,251,119,487,426,843,568,816,863,240,066,240,727,289,815,584,448,288,546,531,420` 次预期尝试 |
| **已发行** | `111900.00000000 SALT`，分布在 `2238` 个未花费输出上 |
| **下一块奖励** | `50.00000000 SALT` |
| **距离下次难度调整** | `2` 块 |
| **距离下次减半** | `207762` 块 |
| **交易数** | `2238` |

### 最近的盐块

| # | 哈希 | 采盐者 | 留言 | 交易 | 奖励 | 时间 |
|--:|---|---|---|--:|--:|---|
| `2237` | `58e1864dffbf71cb133c…` | [@sxxso](https://github.com/sxxso) | `何须再借他人骨` | `1` | `50.00000000` | 2026-09-17 01:02 UTC |
| `2236` | `b295ab8503891e33b1a7…` | [@sxxso](https://github.com/sxxso) | `何须再借他人骨` | `1` | `50.00000000` | 2026-09-17 01:02 UTC |
| `2235` | `d33866d8ead832eb1baf…` | [@sxxso](https://github.com/sxxso) | `何须再借他人骨` | `1` | `50.00000000` | 2026-09-17 01:02 UTC |
| `2234` | `a61357257c243e05414a…` | [@sxxso](https://github.com/sxxso) | `何须再借他人骨` | `1` | `50.00000000` | 2026-09-17 01:02 UTC |
| `2233` | `bd02213220990d234b93…` | [@sxxso](https://github.com/sxxso) | `何须再借他人骨` | `1` | `50.00000000` | 2026-09-17 01:02 UTC |
| `2232` | `5bcd202083e0516f9b79…` | [@sxxso](https://github.com/sxxso) | `何须再借他人骨` | `1` | `50.00000000` | 2026-09-17 01:01 UTC |
| `2231` | `cdaa3171080ba5530898…` | [@sxxso](https://github.com/sxxso) | `何须再借他人骨` | `1` | `50.00000000` | 2026-09-17 01:01 UTC |
| `2230` | `8741b0e3543a29e5236e…` | [@szfzmzx](https://github.com/szfzmzx) | `大佬们别挖了，快休息，让我来挖` | `1` | `50.00000000` | 2026-09-17 00:57 UTC |
| `2229` | `c1ca8d0e869d3b5c60e0…` | [@sxxso](https://github.com/sxxso) | `何须再借他人骨` | `1` | `50.00000000` | 2026-09-17 00:32 UTC |
| `2228` | `869b5da77993d8153777…` | [@sxxso](https://github.com/sxxso) | `何须再借他人骨` | `1` | `50.00000000` | 2026-09-17 00:32 UTC |

### 采盐者

| 采盐者 | 盐块数 | 占比 |
|---|--:|--:|
| [@Heartcoolman](https://github.com/Heartcoolman) | `744` | `33.2%` |
| [@yannicksong0106](https://github.com/yannicksong0106) | `620` | `27.7%` |
| [@sxxso](https://github.com/sxxso) | `377` | `16.8%` |
| [@StevenQinDev](https://github.com/StevenQinDev) | `316` | `14.1%` |
| [@szfzmzx](https://github.com/szfzmzx) | `120` | `5.4%` |
| [@Cheerimy-Studio](https://github.com/Cheerimy-Studio) | `39` | `1.7%` |
| [@ptcy03](https://github.com/ptcy03) | `22` | `1.0%` |

### 持有者

_运行 `python3 saltbox.py identity --handle 你的GitHub用户名` 并把它打印的那行贴到提交处，这里就会显示你的名字。_

| 持有者 | 地址 | 余额 |
|---|---|--:|
| [@Heartcoolman](https://github.com/Heartcoolman) | `salt1qvhmjvm7mrczey63h7yxcpfdr3dj93w638t77p2` | `37200.00000000 SALT` |
| [@yannicksong0106](https://github.com/yannicksong0106) | `salt1q0rvj5waf6esv8rktd9m7kw2qfncv2d0h4u8002` | `31000.00000000 SALT` |
| [@sxxso](https://github.com/sxxso) | `salt1qf9e3eu3qeekvm7kgwyc7d8ylfk8llq7juwx5sq` | `18850.00000000 SALT` |
| [@StevenQinDev](https://github.com/StevenQinDev) | `salt1q5umsmm2dcf3dhgvpw6n6xd2zu7xtnfswmgypew` | `15800.00000000 SALT` |
| [@szfzmzx](https://github.com/szfzmzx) | `salt1q0epfdhc82cnw77h68g8w8us4tmylexkq6s9fsd` | `6000.00000000 SALT` |
| [@Cheerimy-Studio](https://github.com/Cheerimy-Studio) | `salt1q5lzywhrm8rlrrrpgez7ptsvctwf7svytkdzzex` | `1750.00000000 SALT` |
| _未认领_ | `salt1q728yr9vqc7vcjd9kpfelgsphuz56fyxkvp2jf5` | `1100.00000000 SALT` |
| _未认领_ | `salt1qzcegwtpphkd3cee8jq5hvwnc36cjtmxlvhejd8` | `200.00000000 SALT` |

<sub>以上内容由 `chain/blocks.jsonl` 在高度 2237 自动生成，任何人可用 <code>python3 verify.py</code> 自行核验。</sub>

<!-- SALT:END -->

## 什么是盐粒

盐粒不是币。

它借的是盐的那点意思：

- **一粒很小。** 单独一粒盐什么都不是，撒进锅里才有味道。
- **粒粒相同。** 没有哪一粒盐比别的更贵重，也不该有。
- **多了会咸。** 盐的好不在于多，在于刚好。
- **能存住东西。** 盐腌住时间，让一件事留得久一点。

这条链记的就是这样的东西：谁筛出了一粒，谁把一粒交给了谁，谁在那粒盐上写了句话。它不解决任何问题，也不打算有用。它只是一份同时被很多人看着的账本 —— 一个能在 GitHub 上自己长出内容的容器。

「盐粒」这个名字本身就是结论的一部分：越想让它值钱，它就越没有意义。

## 采盐教程：筛出一粒盐

在这个项目里，采盐就是**筛盐**。机器面前有一串数字，要从里面挑出若干，让它们加起来正好等于一个目标重量。挑对了，就算筛出一粒，这一粒记在筛它的人名下。

### 第 0 步：先给仓库点个 ★ Star

没点 Star 的人提交的盐块会被节点拒绝 —— 这是参与的唯一门槛，和账本本身无关。

### 第 1 步：准备 Python

只需要 Python 3.8 以上，不需要安装任何第三方库。

```bash
python3 --version
```

### 第 2 步：把仓库拿下来

```bash
git clone https://github.com/Cheerimy-Studio/Saltgrain.git
cd Saltgrain
```

后面几步的命令都在这个目录里跑。

### 第 3 步：建一个盐框

```bash
python3 saltbox.py new
```

它会打印一个 `salt1…` 开头的地址，那就是你的收盐地址。私钥会明文写在 `saltbox.json` 里 —— 请把它当成一张随时会丢的废纸，**绝不要用它去做任何正经事**。

### 第 4 步：筛一粒

```bash
python3 miner.py --miner 你的GitHub用户名 --message "想留在盐粒上的一句话"
```

`--miner` 必须是你自己的 GitHub 用户名，机器人会核对提交者的身份。采盐可能要几分钟到几十分钟，屏幕上的进度会告诉你大概还剩多久。

想采得快，就去改求解的算法 —— 这个项目比的从来不是机器好不好，而是想法好不好。这是它唯一认真的一件事。

### 第 5 步：交上去

采完之后，终端会打印一行以 `salt-block-v1:` 开头的内容。把它整行复制，贴到这个仓库的 **[采盐提交处](../../issues/53)** 里，发一条评论。

### 第 6 步：等着

机器人会在一两分钟内回复你。成功的话它会说盐块已被接受，上面的账本自己就更新了 —— 不需要你做任何事。失败的话它会告诉你为什么。

## 把盐寄给别人

```bash
python3 saltbox.py send --to 对方的salt1地址 --amount 1.5 --memo "给"
```

它打印一行 `salt-tx-v1:`，贴到 **[寄送提交处](../../issues/2)**。会有人把它打包进下一个盐块。手续费高的先被拿走。刚筛出来的盐要等十个盐块之后才能动，这是规矩。

## 让账本上出现你的名字

账本里默认只有地址。想让别人看到你的名字：

```bash
python3 saltbox.py identity --handle 你的GitHub用户名
```

把打印出的 `salt-id-v1:` 那行贴到 **[寄送提交处](../../issues/2)**。名字只影响显示，不影响谁真正拥有盐粒 —— 那是私钥说了算的事。

## 自己核验整条账本

不用相信这个页面，也不用相信机器人：

```bash
python3 verify.py
```

它会把整条链从头到尾算一遍。通过就打印一串数字，不通过会明确告诉你哪一块有问题。任何人、任何一台机器，跑出来的结果都一样。

## 规则速览

不用看懂也能玩，但知道这些会舒服一点：

- 每采出一粒奖励 50 SALT，每 **210,000 粒**减半一次。能采的粒数一共 **13,440,000 粒**，发完总量约 **21,000,000 SALT**，之后奖励为 0。
- 按每粒 10 分钟的目标算，第一次减半在约 4 年后，全部采完要 200 多年。
- 奖励要等 10 个盐块成熟后才能花。
- 每 16 个盐块，筛盐的难度自动调整一次，人多了就变难，人少了就变简单。
- 一条链同时只有一个有效版本。别人接不上你的盐块，你的那粒就不算数。

## 提醒

- **盐粒一文不值。** 永远不要花钱买它，也不要拿它当抵押、当承诺、当投资。
- 私钥是明文保存的。**别拿它存任何要紧的东西。**
- 这个仓库是唯一的记账人。你能信它，是因为整条链随时可以被任何人重算一遍，不是因为它承诺了什么。

## 目录

```
miner.py        筛盐
saltbox.py      盐框
verify.py       核验
submit.py       节点：处理每一条提交
saltgrain/      账本规则本身
chain/          账本数据
docs/           盐仓
```

MIT License。欢迎 Fork 一锅自己的盐水。

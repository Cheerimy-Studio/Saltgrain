/* Saltgrain (盐粒) block explorer — all consensus logic, ported from the Python reference.
   No dependencies. Everything below runs in the visitor's browser. */

const REPO = document.documentElement.dataset.repo || "Cheerimy-Studio/Saltgrain";
...
/* subset-sum work function (saltgrain/pow.py) */
...
/* address = bech32(hrp="salt", v0, sha256(pubkey)[:20]); addresses minted
   before the rename carry hrp "rofl" and are still valid */
...
/* The 20-byte payload of a salt1… or legacy rofl1… address, or null.
   Ownership compares payloads, so a coinbase paid to a pre-rename address
   does not read as "wrong key" in the explorer. */
function addrPayload(addr) {
  const a = String(addr).toLowerCase();
  const pos = a.lastIndexOf("1");
  if (pos < 1 || pos + 7 > a.length || a.length > 90) return null;
  const hrp = a.slice(0, pos);
  if (hrp !== "salt" && hrp !== "rofl") return null;
  const data = [...a.slice(pos + 1)].map(c => CHARSET.indexOf(c));
  if (data.some(d => d < 0)) return null;
  const expanded = [...hrp].map(c => c.charCodeAt(0) >> 5).concat([0], [...hrp].map(c => c.charCodeAt(0) & 31));
  if (bechPolymod(expanded.concat(data)) !== 1) return null;
  const body = data.slice(0, -6);
  ...
  return out.length === 20 ? out.join(",") : null;
}
function sameAddress(a, b) {
  const pa = addrPayload(a), pb = addrPayload(b);
  ...
  blocks.forEach((b, h) => {
    ...
    b.txs.forEach((t, ti) => {
      ...
      for (const i of t.inputs) {
        ...
        if (!sameAddress(pubToAddress(i.pubkey), u.address)) fail("public key does not match the spent address");
...
function ago(ts) {
  ...
  if (d < 90) return `${d} 秒前`;
  if (d < 5400) return `${Math.floor(d / 60)} 分钟前`;
  if (d < 172800) return `${Math.floor(d / 3600)} 小时前`;
